...
INT_ = GetINT_ABC(ExpDat, iData);

[GC, ~, ~, RM] = GetTiming(ExpDat, GC2RMdat, iData, 1);

INT_Cell = struct2cell(INT_);

INT_sum = zeros(size(INT_Cell{1, 1}));
for i = 1 : 20
    INT_sum = INT_sum + INT_Cell{i, 1};
end

[Itheta, INT] = GetItheta(ExpDat, iData);

for iFrame = 1 : NUM_FRAME
    INT_sum_(:, iFrame) = Itheta(:, :, iFrame) \ INT_sum(:, iFrame);

    INT2(:, iFrame) = Itheta(:, :, iFrame) \ INT(:, iFrame);
end
...
figure
hold on
plot(INT_sum_(14, GC : RM), 'Color', 'r')
plot(IAADatas(1).th2d_(14).tau(19, GC : RM), 'Color', 'b')
plot(INT2(14, GC : RM), 'Color', [0 128/255 0])

figure
hold on 
plot(INT(14, GC : RM))
plot(INT_sum(14, GC : RM))

function [Itheta, INT] = GetItheta(ExperimentDatas, iData)
tic
addpath('../')
...
for iFrame = 1 : NUM_FRAME
    ...
    Lg0l1 = Lg0l(1, iFrame_);
    Lg0l2 = Lg0l(2, iFrame_);
    Lg0l3 = Lg0l(3, iFrame_);
