...
%    16-Jul-2022 18:59:26

mt1 = [0.0;0.0;0.0;g3.*(Lg42.*m4+m4.*(L32-Lg32)+Lg02.*(m1+m2+m3+m4+m0u)+Lg22.*(m2+m3+m4)+(L12-Lg12).*(m2+m3+m4)+L0d2.*(m1+m2+m3+m4)+Lg12.*(m1+m2+m3+m4)+Lg32.*(m3+m4)+(m3+m4).*(L22-Lg22))-g2.*(Lg43.*m4+m4.*(L33-Lg33)+Lg03.*(m1+m2+m3+m4+m0u)+Lg23.*(m2+m3+m4)+(L13-Lg13).*(m2+m3+m4)+L0d3.*(m1+m2+m3+m4)+Lg13.*(m1+m2+m3+m4)+Lg33.*(m3+m4)+(m3+m4).*(L23-Lg23))];
